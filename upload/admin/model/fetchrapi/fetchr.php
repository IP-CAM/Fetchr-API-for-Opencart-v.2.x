...
class ModelFetchrapiFetchr extends Model {
	...
	public function getPaymentMethod($order_id, $payment_code)
	{
		if ($payment_code == 'GOP_COD') {
			$query = $this->db->query("SELECT value FROM " . DB_PREFIX . "order_total WHERE code = 'GOP_COD_Fee' AND order_id = '" . (int)$order_id . "'");
